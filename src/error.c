...
static const char *get_severity_color(ErrorSeverity severity) {
    switch (severity) {
        case ERROR_SEVERITY_FATAL:   return COLOR_RED;
        case ERROR_SEVERITY_ERROR:   return COLOR_RED;
        case ERROR_SEVERITY_WARNING: return COLOR_YELLOW;
        case ERROR_SEVERITY_INFO:    return COLOR_CYAN;
        default:                     return COLOR_RESET;
    }
}

void error_report(const char *filename, int line_number, int column,
                 const char *line_content, ErrorSeverity severity,
                 const char *format, ...) {
    va_list args;
